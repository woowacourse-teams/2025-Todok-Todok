...
public class CommentCommandService {
    ...
    private void validateDuplicatedReport(
        ...
        if (commentReportRepository.existsByMemberAndComment(member, comment)) {
            throw new IllegalArgumentException(String.format("이미 신고한 댓글입니다: %d -> %d", member.getId(), comment.getId()));
        }
    }
    ...
    private void validateCommentMember(
        ...
        if (!comment.isOwnedBy(member)) {
            throw new IllegalArgumentException(String.format("자기 자신의 댓글만 수정/삭제 가능합니다: member-%d, comment-%d",
    ...
    private void validateHasReply(final Comment comment) {
        if (replyRepository.existsByComment(comment)) {
            throw new IllegalArgumentException(String.format("대댓글이 존재하는 댓글은 삭제할 수 없습니다: %d", comment.getId()));
        }
    }
}
