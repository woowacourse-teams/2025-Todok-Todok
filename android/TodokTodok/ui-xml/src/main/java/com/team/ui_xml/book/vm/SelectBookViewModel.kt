package com.team.ui_xml.book.vm
...
import com.team.ui_xml.book.SearchedBookStatus
import com.team.ui_xml.book.SelectBookUiEvent
import com.team.ui_xml.book.SelectBookUiState
...
class SelectBookViewModel
    ...
    constructor(
        ...
        fun changePageSize(size: Int) {
            setState { SelectBookUiState(pageSize = size) }
        }
        ...
        fun updateKeyword(value: String) {
            if (value.isBlank() || value.isEmpty()) {
                setState { SelectBookUiState(keyword = null) }
                return
            }
            setState { SelectBookUiState(keyword = Keyword(value)) }
        }
        ...
        fun addSearchedBooks() {
            ...
            setState { SelectBookUiState(status = SearchedBookStatus.Loading) }
        ...
        private fun updateSearchedBooks(value: String) {
            setState {
                SelectBookUiState(
                    status = SearchedBookStatus.Loading,
            ...
            bookRepository
                ...
                .onSuccess { searchedBooksResult ->
                    if (searchedBooksResult.books.isEmpty()) {
                        setState { SelectBookUiState(status = SearchedBookStatus.NotFound) }
                        return@onSuccess
                    }
                    setState {
                        SelectBookUiState(
                    ...
                    setState { SelectBookUiState(status = SearchedBookStatus.NotStarted) }
