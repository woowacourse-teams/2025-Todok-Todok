...
import com.team.data.network.response.discussion.page.ActivatedDiscussion
...
class FakeDiscussionRemoteDataSource : DiscussionRemoteDataSource {
    ...
    override suspend fun getLatestDiscussions(
        ...
        val startIndex = cursor?.toIntOrNull() ?: 0
        val endIndex = (startIndex + size).coerceAtMost(LATEST_DISCUSSIONS_RESPONSE_FIXTURE.size)
        val pageDiscussions = LATEST_DISCUSSIONS_RESPONSE_FIXTURE.subList(startIndex, endIndex)

        val hasNext = endIndex < LATEST_DISCUSSIONS_RESPONSE_FIXTURE.size
        val nextCursor = if (hasNext) endIndex.toString() else ""
