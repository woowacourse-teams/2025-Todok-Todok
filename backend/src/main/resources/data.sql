...
INSERT INTO COMMENT (content, member_id, discussion_id, created_at, modified_at)
VALUES
('객체 지향이 아닌 절차 지향 언어로 영화 예매 시스템을 구현했다면 어땠을 지를 생각해보면 좋을 것 같아요.\n\n우리는 현실 세계를 객체로 추상화하여 바라볼 수 있는 능력을 지니고 있습니다.\n하지만 설계와 구현에 들어서는 순간부터는 객체 관점을 유기하고 절차 지향적인 코드를 떠올려야 하겠죠?\n\n객체 지향 언어를 사용한다면 우리가 바라본 객체의 세계를 기반으로 설계하고, 더 나아가 구현까지 할 수 있습니다.\n객체 세계를 절차 지향적으로 옮겨 놓은 것보다는 객체 관점 그대로 구현한 코드가 훨씬 읽기 쉽겠죠?\n\n그러한 의미에서 객체 지향 패러다임이 매끄럽게 연결될 수 있고 강력하다고 생각해요.',
6, 5, CURRENT_TIME, CURRENT_TIME);

INSERT INTO COMMENT (content, member_id, discussion_id, created_at, modified_at)
VALUES
('해당 내용에서 다루는 객체 지향의 강력함은\n"요구사항을 분석하는 초기 단계부터 프로그램을 구현하는 마지막 단계까지 객체라는 동일한 추상화 기법을 사용한다" 라고 생각합니다.\n이는 다시 말해, 객체 지향은 요구사항과 구현체의 단위가 같아 직관적이란 의미입니다.\n\n아래 절차 지향과 비교하면 좀 더 와닿을 것 같습니다.\n\n"계좌는 통장을 개설해 입금, 출금 기능을 수행할 수 있다" 라는 요구 사항이 주어졌을 때,\n\n절차 지향의 경우\n은행 계좌라는 비즈니스 개념과 그에 대한 처리 로직(입금, 출금, 송금 등)이 완전히 분리되어 있습니다.\n계좌는 단순한 데이터 저장소에 불과하고, 모든 기능은 외부 함수로 구현됩니다.\n\n이는 하나의 요구사항을 다수의 개별 단위로 나눈다는 의미입니다.\n\n이러한 경우가 요구사항과 실제 구현의 단위가 달라졌기에,\n서비스가 복잡해질수록 요구 사항과 실제 구현이 크게 달라지게 되고, 요구사항에 맞는 구현 지점을 찾기 어려워집니다.\n다시 말해, 개발의 직관성이 떨어지게 됩니다.\n\n객체 지향의 경우\n반면 객체 지향은 "계좌는 입금, 출금 기능을 수행할 수 있다"는 현실 개념이 코드에 그대로 반영됩니다.\n계좌 객체가 자신의 상태(잔액)를 관리하고 행동(입출금)을 수행합니다.\n\n이는 요구사항 분석 단계에서 "계좌는 입금, 출금, 송금 기능을 수행할 수 있다"라고 명세한 내용이 그대로 코드로 표현됨을 의미합니다.\n때문에, 절차 지향의 경우보다 요구사항에 맞는 구현 지점을 직관적으로 찾을 수 있습니다.\n\n이러한 차이가 작성자분이 질문한 구문에서 말하는 객체지향의 강력한 점이 아닐까 생각합니다.',
...
INSERT INTO COMMENT (content, member_id, discussion_id, created_at, modified_at)
VALUES
('다시 생각해보니 외부에서 객체를 많이 사용한다고 해서 결합도가 높아진다는 것은 뭔가 안맞는다는 생각이 들어요!\n물론 객체 간 의존성이 추가되기 때문에 약간의 결합도가 증가할 수는 있다고 생각해요.\n\n하지만 객체의 응집도가 높다면, 책임이 새어나가지 않았을 뿐만 아니라 내부 구현이 잘 캡슐화되었을텐데요.\n그렇다면 코드에 변경이 생기더라도 외부로 변경이 전파되지 않을테니 결합도가 낮지 않을까요?\n\n그렇기 때문에 우리가 말하고 있는 응집도와 결합도 측면에서 높은 응집도, 높은 결합도라는 것이 존재할 수 있는 지 궁금해요.🤔',
 3, 8, CURRENT_TIME, CURRENT_TIME);

INSERT INTO COMMENT (content, member_id, discussion_id, created_at, modified_at)
VALUES
('일반적인 관점과 변경의 관점 중에서 책에는 변경의 관점이 중요하다고 설명되어 있어서 후자로 설명해 보겠습니다.\n\n책의 내용은 아래와 같이 설명했습니다.\n\n- 변경의 관점에서 응집도와 결합도\n응집도란 변경이 발생할 때 모듈 내부에서 발생하는 변경의 정도로 측정할 수 있다.\n결합도는 한 모듈이 변경되기 위해서 다른 모듈의 변경을 요구하는 정도로 측정할 수 있다.\n\nEx) 카페 카운터 점원(높은 응집도, 낮은 결합도)\n가벼운 예시로 설명을 해보겠습니다.\n\n한 커피 매장의 카운터 역할(결제 담당)을 가진 점원(CoffeCounterService) 이 있습니다.\n\n해당 점원의 역할은 결제의 구체적인 임무를 가지고 있습니다.\n\n점원(CoffeCounterService)\n- 계산\n- 영수증 발급\n- 포인트 적립\n\n위와 같은 상황에 다음 날, 아침에 갑자기 계산 과정의 거스름돈을 포인트로 모두 전환해야 합니다.\n\n이런 상황에서 변경의 요소는 점원에게\n\n점원(CoffeCounterService)\n- 계산(거스름돈 삭제)\n- 영수증 발급(영수증 작성에 포인트 전환 내용 추가 표시)\n- 포인트 적립(거스름돈만큼 추가로 지급)\n와 같이 점원 하나의 존재 변경으로 아침의 상황이 마무리되었습니다. ☺️\n\n그럼, 반대의 경우는 어떨까요? 🤔\n\nEx) 카페 카운터 점원(낮은 응집도, 높은 결합도)\n점원의 기능이 방대하다고 느껴서 분리했습니다.\n\n점원(CoffeCounterService)\n- 계산(CoffeCounterPaymentService)\n- 영수증 발급(CoffeCounterReceiptService)\n- 포인트 적립(CoffeCounterPointService)\n그럼 점원은 세 가지의 Service 모두 호출해 사용하고 있습니다.\n\n얼핏 보면 더 잘게 분리되어서 유용하고 명확하다고 느낄 수 있지만, 🤔 이전과 같이 똑같은 상황을 마주해봅시다.\n\n요구 사항 등장 : 거스름돈 모두 포인트로 변경해 ‼️\n\n그럼 변경 요소가\n- 계산\n- 영수증 발급\n- 포인트 적립\n각각 클래스에 고스란히 변경이 발생합니다. 이뿐만이 아닙니다.\n\n모든 것을 담당해 호출하는 점원에게도 영향이 고스란히 파급효과로 이어집니다.\n\n이런 상황을 응집도가 낮고 결합도가 높다고 생각합니다.\n\n또한, 이 학습을 하면서 저자가 말하는 트레이드 오프의 의미를 파악했습니다.\n\n이전 상황에\n점원(CoffeCounterService) 은 세 가지 기능을 담당했습니다. 그럼 이런 의문도 듭니다.\n\n"계산, 영수증 발급, 포인트 적립 세 가지 역할을 가지고 있는 거 아닌가?"\n\n결국 이런 상황에서는 어디까지를 하나의 역할로 묶고, 어디부터 분리할 것인지를 설계자가 스스로 판단하고 결정해야 한다는 의미가 아닐까 생각했습니다.\n\n즉, 기능을 세부적으로 나누어 역할을 분리할지, 아니면 어느 정도 관련된 기능들은 하나의 역할로 묶을지 트레이드오프를 고려해야 한다는 뜻으로 이해했습니다.',
 5, 8, CURRENT_TIME, CURRENT_TIME);
