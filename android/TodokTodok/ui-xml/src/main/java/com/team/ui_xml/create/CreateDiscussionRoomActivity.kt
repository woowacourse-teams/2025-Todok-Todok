...
import com.team.ui_xml.create.vm.CreateDiscussionRoomViewModel
import com.team.ui_xml.databinding.ActivityCreateDiscussionRoomBinding
import com.team.ui_xml.discussiondetail.DiscussionDetailActivity
import com.team.ui_xml.draft.DraftsActivity
...
class CreateDiscussionRoomActivity : AppCompatActivity() {
    ...
    private fun settingEditMode(binding: ActivityCreateDiscussionRoomBinding) {
        binding.apply {
            etDiscussionRoomTitle.setText(viewModel.uiState.value?.title)
            etDiscussionRoomOpinion.setText(viewModel.uiState.value?.opinion)
            btnCreate.text = getString(R.string.create_discussion_edit)
    ...
    private fun navigateToSelectBook() {
        val intent = SelectBookActivity.Companion.Intent(this@CreateDiscussionRoomActivity)
    ...
    private fun showDraftDialog() {
        ...
        supportFragmentManager.setFragmentResultListener(
            DraftDialog.Companion.KEY_REQUEST_DRAFT,
            this,
        ) { _, bundle ->
            val isSave = bundle.getBoolean(DraftDialog.Companion.KEY_RESULT_DRAFT)
    ...
    private fun showDraftsListDialog() {
        val intent = DraftsActivity.Companion.Intent(this)
        ...
        viewModel.uiEvent.observe(this@CreateDiscussionRoomActivity) { event ->
            when (event) {
                ...
                is CreateDiscussionUiEvent.SaveDraft -> {
                    if (event.possible) {
                        val dialog =
                            CommonDialog.newInstance(
                                getString(R.string.create_discussion_draft_message),
                                getString(R.string.create_discussion_draft),
                            )
                    ...
                    val dialog =
                        CommonDialog.newInstance(
                            getString(R.string.create_discussion_no_exist_file),
                            getString(R.string.create_discussion_overload),
                        )
                    dialog.show(supportFragmentManager, CommonDialog.TAG)
                }
    ...
    private fun navigateToDiscussionDetail(
        ...
        val intent =
            DiscussionDetailActivity.Companion
                .Intent(
                    this@CreateDiscussionRoomActivity,
                    discussionRoomId,
                    mode,
        ...
        fun Intent(
            ...
            when (mode) {
                is SerializationCreateDiscussionRoomMode.Create ->
                    intent.apply {
                        putExtra(CreateDiscussionRoomViewModel.Companion.EXTRA_MODE, mode)
