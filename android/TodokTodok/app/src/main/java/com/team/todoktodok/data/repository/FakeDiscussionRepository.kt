...
class FakeDiscussionRepository(
    private val fakeDataSource: DiscussionRemoteDataSource,
) : DiscussionRepository {
    override suspend fun getLatestDiscussions(
        size: Int,
        cursor: String?,
    ): NetworkResult<LatestDiscussionPage> = fakeDataSource.getLatestDiscussions(size, cursor).map { it.toDomain() }
